...
public class TDWToMySqlRunner extends TDWDDCTaskRunner {
	protected static final String ONE_OFF = "O";
	protected static final String CYCLE_UNIT_DEFAULT = "D";

	public static void main(String[] args) {
		TaskRunnerLoader.startRunner(TDWToMySqlRunner.class, (byte) 98);
	}
	...
	public void execute() throws IOException {
		...
		if (task.getSourceServersCount() < 3) {
			...
			keyValues.put("task_desc",
					"Task server configuration is incorrect, expect at least 3 source "
		...
		LServer dbServer = task.getTargetServers(0);
		...
		try {
			...
			partitionValue = (partitionValue == null ? "" : RunnerUtils.replaceDateExpr(
					partitionValue, runDate));
		}
		catch (Exception e) {
		...
		String cycleUnit = task.getCycleUnit().trim();
		if (ONE_OFF.equalsIgnoreCase(cycleUnit)) {
			String subCycleUnit = this.getExtPropValue("subCycleUnit");
			if (!StringUtils.isBlank(subCycleUnit) && !ONE_OFF.equalsIgnoreCase(subCycleUnit)) {
				cycleUnit = subCycleUnit;
			}
		}
		SimpleDateFormat dateFormat = getDateFormat(this.getExtPropValue("offRange"),
				this.getExtPropValue("offRangeType"), cycleUnit);
		if (dateFormat == null) {
		...
		if (APPEND_FLAG.equalsIgnoreCase(loadMode)) {
			...
			if ((partitionValue == null) || (partitionValue.length() < 2)) {
				...
				/* NOTE:Only works on mysql 5.5 or later version */
				truncateSQL = "ALTER TABLE " + dbTableName + " TRUNCATE PARTITION "
						+ partitionValue;
			}
		}
		this.writeLocalLog(Level.INFO, "redo_flag:" + task.getRedoFlag());
		...
		while (st.hasMoreTokens()) {
			...
			if (countTokens == 1) {
				...
				if (secondToken.startsWith("char")) {
					...
					/* Do nothing for date. */
					// String dbDateFormat = "";
					// if ("date".equalsIgnoreCase(secondToken) ||
					// "timestamp".equalsIgnoreCase(secondToken)) {
					...
					// sb2.append("TO_TIMESTAMP(?, '" +
					// dbDateFormat.trim().toUpperCase() + "'),");
					// }
				}
			}
		}
		...
		// insert sql--------------
		String mysqlSQLTmp = "INSERT INTO " + dbTableName + "(" + columnNames + ") VALUES ("
				+ columnValues + ")";
		// -------------------

		String mysqlSQL = this.getExtPropValueWithDefault("dbSQL", mysqlSQLTmp);
		this.writeLocalLog(Level.INFO, "mysqlSQL: " + mysqlSQL);
		...
		final String DX_TEMPLATE_PATH = this.requestGlobalParameters()
				.get("dx_template_hdfs_to_db");
		...
		String externalTableDataPath = tmpPathPrefix + File.separator + task.getId() + 
				                       File.separator + SECOND_FORMAT.format(runDate) +  
		...
		try {
			...
			if (!createExternalTableWithSQL(plcProgram, hiveServer, nameNodeServer,
					externalTableDataPath, tdwDbName, externalTableDb, externalTableName,
			 ...
			 * node.
			 */
			LoadDataResult loadDataResult = loadDataWithSQL(plcProgram, hiveServer, tdwDbName,
			...
			String hadoopJobUgi = nameNodeServer.getUserName() + ","
					+ nameNodeServer.getUserGroup();
			String user = mysqlUser;
			String password = DxDESCipher.EncryptDES(mysqlPasswd);
			...
			mergekeyValue(sb, "target.path", mysqlSQL);
			...
			mergekeyValue(sb, "driver.string", "com.mysql.jdbc.Driver");
			...
			if (APPEND_FLAG.equalsIgnoreCase(loadMode)) {
				ignoreErrorsSql.append("ALTER TABLE ").append(dbTableName)
						.append("  ADD COLUMN etl_stamp varchar(200) DEFAULT NULL");
				...
				// ignoreErrorsSql.append(";ALTER TABLE ").append(dbTableName).append(" ADD INDEX ")
				// .append(dbTableNameWithoutSchema).append("_etl_stamp_idx (etl_stamp)");
			}
			...
			int failRead = dbObject.getInt("failed_read");
			int successRead = dbObject.getInt("success_read");
			int successWrite = dbObject.getInt("success_writed");
			int failWrite = dbObject.getInt("failed_writed");
			String taskDesc = dbObject.getString("task_desc");

			/* Generate the TDWTomysql output. */
			...
			if (exitValue != 0) {
				writeLocalLog(Level.SEVERE, "Load data to mysql using DX failed: " + taskDesc);
			...
			float failWritePercentage = (failRead + successRead - successWrite)
					/ ((float) (failRead + successRead));
			...
			if (failWritePercentage > new Float(errorPercentage)) {
				String failMessage = "The failure percentage is beyond: " + errorPercentage
						+ ", the task fails";
			...
			keyValues.put("task_desc", "Load data from TDW to mysql succeeds");
			success = true;
		}
		catch (Exception e) {
