...
public class TDWToOracle10gRunner extends TDWDDCTaskRunner {
	public static void main(String[] args) {
		TaskRunnerLoader.startRunner(TDWToOracle10gRunner.class, (byte) 89);
	}
	...
	public void execute() throws IOException {
		...
		/*
		 * Check whether we need to skip the weekly task for those USP weekly and monthly tasks.
		 */
		...
		if (!oracleTableName.contains(".")) {
			keyValues.put("exit_code", RunnerUtils.ORACLE_TABLE_NAME_ERROR_CODE);
			keyValues.put("task_desc", "Oracle target table name should contain the schema name");
			commitJsonResult(keyValues, false, "");
			return;
		}
		...
		SimpleDateFormat dateFormat = getDateFormat(this.getExtPropValue("offRange"),
				this.getExtPropValue("offRangeType"), task.getCycleUnit().trim());
		if (dateFormat == null) {
		...
		if (APPEND_FLAG.equalsIgnoreCase(loadMode)) {
			truncateSQL = "DELETE FROM " + oracleTableName + " WHERE ETL_STAMP='" + etlTime + "'";
		}
			...
			if ((partitionValue == null) || (partitionValue.length() < 2)) {
				...
				truncateSQL = "ALTER TABLE " + oracleTableName + " TRUNCATE PARTITION " + partitionValue;
			}
		}

		/* Generate the SQL based on the oracle column names. */
		...
		while (st.hasMoreTokens()) {
			...
			if (countTokens == 1) {
				...
				if (secondToken.startsWith("char")) {
					int charLength = new Integer(secondToken.substring(secondToken.indexOf("(") + 1,
							secondToken.indexOf(")")));
					sb1.append(columnName).append(",");
					sb2.append("SUBSTRB(?,1," + charLength + "),");
				}
		...
		String oracleSQLTmp = "INSERT INTO @{DX_TEMP_TABLENAME} (" + columnNames + ") VALUES (" + columnValues + ")";
		String oracleSQL = this.getExtPropValueWithDefault("oracleSQL", oracleSQLTmp);
		...
		if (CommonUtils.isBlankAny(DX_TEMP_PATH, DX_HOME, DX_TEMP_SCHEMA, DX_TEMPLATE_PATH)) {
			throw new IOException(String.format("Empty value found: dx_temp_path=%s, "
					+ "dx_home=%s, dx_temp_schema=%s,dx_template_hdfs_to_oracle=%s", DX_TEMP_PATH, DX_HOME,
					DX_TEMP_SCHEMA, DX_TEMPLATE_PATH));
		}
...
//			HadoopResult createResult = RunnerUtils.createHDFSPath(hadoopCommand, nameNodeServer,
...
//						+ "external table fails with error code: " + createResult.getHadoopReturnCode());
...
//			}
			...
			LoadDataResult loadDataResult = loadDataWithSQL(plcProgram, hiveServer, tdwDbName, externalTableDb,
					externalTableName, filterSQL, specialParam);
			...
			if (exitValue != 0) {
				...
				keyValues.put("task_desc", "Load data to Oracle using DX failed");
				keyValues.put("run_date", HOUR_FORMAT.format(runDate));
			...
			/* Parse the DX output. */
			JSONObject oracleObject = new JSONObject(output);
			int failRead = oracleObject.getInt("failed_read");
			int successRead = oracleObject.getInt("success_read");
			int successWrite = oracleObject.getInt("success_writed");
			int failWrite = oracleObject.getInt("failed_writed");

			/* Generate the TDWToOracle output. */
			...
			float failWritePercentage = (failRead + successRead - successWrite) / ((float) (failRead + successRead));
			failWritePercentage = failWritePercentage * 100;

			/*
			 * The whole process succeeds if the fail write percentage is below the error percentage, otherwise we think
			 * it fails.
			 */
			...
			keyValues.put("task_desc", "Load data from TDW to Oracle succeeds");
			success = true;
		}
		catch (Exception e) {
