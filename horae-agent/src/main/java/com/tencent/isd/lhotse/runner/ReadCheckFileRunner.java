...
public class ReadCheckFileRunner extends DDCTaskRunner {	
	...
	public void execute() throws 
		...
		try {
			...
			try {
			    ...
			    checkMethod =
				    	RunnerUtils.formatStrDateParam(this.getExtPropValue("checkMethod"), runDate);
			} catch (Exception e) {
			...
			if (HDFS_SYSTEM.equalsIgnoreCase(sourceFileSystemType)) {
				checkResult = checkHdfsSource(sourceFileServer, sourceFilePath, sourceFileName, 
                        checkFilePath, checkFileName, checkLevel,checkMethod);
	...
	private CheckResult checkHdfsSource(LServer sourceServer, 
		...
		this.writeLocalLog(Level.INFO, hadoopCommand+" "+sourceServer.getHost()+"  "+checkFilePath);
        ...
        //--add hadoop api  branch(jessicajin)
        ListHdfsDirResult checkDirResult = null;
        CatFileResult catResult = null;
        if(checkMethod!=null&&"api".equals(checkMethod.toLowerCase())){
        	this.writeLocalLog(Level.INFO,"-----hadoop api branch---");
        	checkDirResult =  
        			RunnerUtils.getHdfsFileDescriptorsApi(sourceServer, checkFilePath, checkFileName, this);
        	catResult = 
        			RunnerUtils.catHdfsFileApi(sourceServer, checkFilePath,
        					checkDirResult.getDescriptors().get(0).getFileName(), this);
        }else{
            checkDirResult =
		...
		//-------add hadoop api branch(jessicajin)
		ListHdfsDirResult dirResult = null;
		if (checkMethod!=null&&"api".equals(checkMethod.toLowerCase())) {
			this.writeLocalLog(Level.INFO,"-----hadoop api branch---");
			dirResult = RunnerUtils.getHdfsFileDescriptorsApi(sourceServer,
					sourceFilePath, sourceFileNamePattern, this);
		} else {
			dirResult = RunnerUtils.getHdfsFileDescriptors(hadoopCommand,
					sourceServer, sourceFilePath, sourceFileNamePattern, this);
		}
		//------------------------
