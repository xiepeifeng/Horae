 ...
 */
public class OracleToHdfsRunnerBK extends AbstractTaskRunner {

    public static void main(String[] args) {
        TaskRunnerLoader.startRunner(OracleToHdfsRunnerBK.class, (byte) 20);
    }
...
//            commitJsonResult(keyValues, false, "");
        ...
        try {
            ...
            hiveDB = CommonUtils.standardizeDateString(this.getExtPropValue("hive.database"),
                    task.getCurRunDate());
            hivePartition = CommonUtils.standardizeDateString(this.getExtPropValue("hive.partition.value"),
                    task.getCurRunDate());
            hiveSubPartition = CommonUtils.standardizeDateString(this.getExtPropValue("hive.sub.partitions"),
                    task.getCurRunDate());
            hiveFields = CommonUtils.standardizeDateString(this.getExtPropValue("hive.field.list"),
                    task.getCurRunDate());
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        String hadoopJobUgi = hdfsUser + "," + hdfsGroup;
        ...
        this.commitTask(LhotseObject.LState.SUCCESSFUL, "Oracle2HDFS runner", "Oracle2HDFS runner finished successfully.");
