...
public abstract class LhotseSubProcess {
	...
	public void startProcess(boolean printInfo) throws Exception {
		...
		errThread = 
			new RunnerOutputThread(es, printInfo, errExitLatch, runner);
		errThread.start();

		normalThread = 
			new RunnerOutputThread(os, printInfo, normalExitLatch, runner);
		normalThread.start();
