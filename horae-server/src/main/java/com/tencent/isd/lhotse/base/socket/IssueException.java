...
public class IssueException extends Exception {
	...
	static {
		// English Desc
		/*
		descriptions.put(TYPE_NOT_REGISTERED, "Task type is not registered");
		descriptions.put(REQUESTS_FULL, "Request queue is full");
		descriptions.put(DATABASE_ERROR, "Database connection is failed");
		descriptions.put(NO_TASK_FOUND, "No task of this type is found");
		descriptions.put(NO_INSTANCE_FOUND, "No instance of this type is found");
		descriptions.put(RUNNER_NOT_AVAILABLE, "Runner is not available");
		descriptions.put(INVALID_RUNNER_STATUS, "Runner status is not normal");
		descriptions.put(INVALID_RUN_DATE, "Run date is invalid");
		descriptions.put(INVALID_TASK_STATUS, "Task status is not normal");
		descriptions.put(BROKER_NOT_MATCHED, "Request broker is not matched with task's broker");
		descriptions.put(ILLEGAL_PRIORITY, "Priority is illegal");
		descriptions.put(PRIORITY_NOT_MATCHED, "Priority is not matched with priority limit of request broker");
		descriptions.put(EXECUTE_TIME_NOT_ARRIVED, "Excuting time has not arrived");
		descriptions.put(DELAY_TIME_NOT_ARRIVED, "Delay time has not arrived");
		descriptions.put(STARTUP_TIME_NOT_ARRIVED, "Startup time has not arrived");
		descriptions.put(ILLEGAL_SELF_DEPEND, "Self-depend of task is illegal");
		descriptions.put(NOT_EARLIEST_INSTANCE, "Self-depend of task is 1, but the instance is not the earliest");
		descriptions.put(TRY_OVER, "Instance has been tried too many times");
		descriptions.put(RETRY_WAIT_NOT_ARRIVE, "Time interval to retry has not arrived");
		descriptions.put(BROKER_FULL, "Too many tasks are running on request broker");
		descriptions.put(SERVER_NOT_AVAILABLE, "Server of this task is not available");
		descriptions.put(SERVER_FULL, "Too many tasks are running on the server of this task");
		descriptions.put(TASK_FULL, "Too much instances of this task are running");
		descriptions.put(HAS_RUNNING_INSTANCE, "There is running instance of this task");
		descriptions.put(PARENT_TASK_NOT_FOUND, "Parent task are not found");
		descriptions.put(PARENT_INSTANCE_NOT_FOUND, "Instances of parent task are not found");
		descriptions.put(PARENT_NOT_SUCCESSFUL, "There is unsuccessful instance of parent tasks");
		descriptions.put(PARENT_CURRENT_DATE_NOT_MATCHED, "Start date of parent task is not matched");
		descriptions.put(PARENT_NEXT_DATE_NOT_MATCHED, "End date of parent task is not matched");
		descriptions.put(INSTANCE_NOT_ENOUGH, "Instances of parent tasks are not enough");
		descriptions.put(INVALID_DEPEND_TYPE, "Dependence type is invalid");
		*/
		// Chinese Desc
	...
	private int state;
	private boolean isByTask;

	public IssueException(byte code, String value) {
		this.issueCode = code;
		this.errorValue = value;
		isByTask = false;
	}

	public IssueException(byte code, String taskId, java.sql.Timestamp runDate, int state) {
		this.issueCode = code;
		this.taskId = taskId;
	...
	public String getMessage() {
		if (!isByTask) {
			...
			return getIssueCodeDescription(issueCode) + ",task:" + taskId + ",date:"
					+ LDateUtil.getUTCDateString(runDate) + ",state:" + state;
		}
	}
